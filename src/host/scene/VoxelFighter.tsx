...
export default function VoxelFighter({ color, walkRef, scale = 1, glow = 0.55, frozen = false, flashRef, dimRef }: VoxelFighterProps) {
  ...
  const mainMat = useMemo(
    () => new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: glow, roughness: 0.4, metalness: 0.2 }),
    [color, glow],
  )
  const darkMat = useMemo(() => new THREE.MeshStandardMaterial({ color: '#12131f', roughness: 0.7, metalness: 0.3 }), [])
  const visorMat = useMemo(
    () => new THREE.MeshStandardMaterial({ color: '#ffffff', emissive: '#ffffff', emissiveIntensity: 1.6, roughness: 0.2 }),
    [],
  )
    ...
    <group scale={scale}>
      <group ref={body}>
        ...
        <mesh ref={leftLeg} position={[0, 0.7, -0.2]} material={darkMat}>
          ...
          <mesh position={[0, -0.35, 0]} material={mainMat}>
        ...
        {/* chest plate */}
        <mesh position={[0.29, 1.2, 0]} material={darkMat}>
        ...
        {/* antenna */}
        <mesh position={[0, 2.42, 0]} material={mainMat}>
